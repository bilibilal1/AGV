...
byte trig = A8;                                    //Assign the ultrasonic sensor pins
byte echo = A9;
...
byte motorSpeed = 105;                             //The maximum motor speed
...
void setup() 
{
  ...
  servoLook.attach(9);                           //Assign the servo pin
  pinMode(trig,OUTPUT);                           //Assign ultrasonic sensor pin modes
  pinMode(echo,INPUT);
  pinMode(22,INPUT); //Ir sensor front
  pinMode(23,INPUT); //Ir sensor back
}
...
void loop() 
{
  ...
  if((distance >= stopDist) && (digitalRead(22)==1) )                        //If there are no objects within the stopping distance, move forward
  {
    moveForward();
